...
- DeepSeek Harness `0.1.0-rc.7`、`0.1.0-rc.8`、`0.1.1-rc.1`、`0.1.1-rc.2`
  或当前官方 `0.1.2-alpha.5`、`0.1.2-rc.1`、`0.1.3-alpha.1` 最新三版预发布通道，并且官方 `dsh` CLI 可用；
...
`0.8.15` 同时读取官方 GitHub Releases 与 npm 已发布版本，解决 GitHub 已发布 `0.1.3-alpha.1`、npm 仍停留 `0.1.2-rc.1` 时漏报新版的问题。最新三版窗口为 `0.1.2-alpha.5`、`0.1.2-rc.1`、`0.1.3-alpha.1`；npm 包未发布时明确提示，不生成不可执行的 npm 升级命令。任一权威来源失败时不推进自动兼容决策；精确兼容门仍要求窗口内至少一个版本兼容，安装、启动、卸载、回滚证据分别记录。Registry CI 现在也覆盖运行代码与自动化脚本变更，并执行完整检查。新版本已通过官方源码构建宿主的隔离安装与运行接口检查；公网发布和真实 Profile 验收仍是独立阶段。
