...
- DeepSeek Harness `0.0.1-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8` 或 `0.1.1-rc.1`，并且官方 `dsh` CLI 可用；
...
```bash
dsh plugin --profile web add 'git+https://github.com/AI-Scarlett/dsh-safe-plugin-manager.git#5e74177e473b8ab71b8d4b030872ae9523969013'
```
...
`0.8.0` 保留每页 24 条的有界加载，并将兼容矩阵扩展到 `0.1.1-rc.1`。最新版本使用完整键避免与早期 rc.1 混淆；商城优先显示 rc.1 兼容且来源较新的插件，旧版本和未知证据仍可见且不会被误标为已验证。
...
- 默认按照 approved、`0.1.1-rc.1` 兼容性、GitHub 固定来源更新时间、推荐和安装量依次排序；不支持当前版本、证据未知或长期未更新的条目依次排在后面；
...
- DSH rc.5–`0.1.1-rc.1` 中使用同一套官方 `ModuleLoader`、`settings.plugins.tab`、Slots 和 Modal 契约；rc.5 的真实包版本是 `0.0.1-rc.5`，新 rc.1 使用完整版本键避免歧义；
...
- 对 rc.5、rc.6、rc.7、rc.8、`0.1.1-rc.1` 分别展示兼容声明，以及安装、启动、卸载、回滚四项运行证据；缺少真实验收时显示未知；
...
| DSH 0.1.1-rc.1 兼容 `0.8.0` | 本地待发布 | 新增无歧义 `0.1.1-rc.1` 矩阵、最新版本排序、旧目录缺键降级和 400 条目录的保守兼容状态迁移。 |
...
不可安装示例；完整目录、`0.1.1-rc.1` 兼容性和来源更新时间排序以
...
- 分别在一次性 DSH `0.0.1-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8`、`0.1.1-rc.1` Profile 中使用官方 CLI 验证安装、配置合成和功能可见性；
