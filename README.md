...
- DeepSeek Harness `0.1.0-rc.7`、`0.1.0-rc.8`、`0.1.1-rc.1`、`0.1.1-rc.2`
  或当前官方 `0.1.3-alpha.2`、`0.1.5-alpha.1`、`0.1.5-alpha.2` 最新三版预发布通道，并且官方 `dsh` CLI 可用；
...
`0.8.16` 继续同时读取官方 GitHub Releases 与 npm 已发布版本，并把 `dsh-v0.1.5-alpha.2`（Commit `b2e3b2a0125854567a4a5fcba75782e42fe84901`）纳入官方版本权威。当前最新三版窗口为 `0.1.3-alpha.2`、`0.1.5-alpha.1`、`0.1.5-alpha.2`；npm 尚未发布的版本明确提示，不生成不可执行的 npm 升级命令。任一权威来源失败时不推进自动兼容决策；精确兼容门仍要求窗口内至少一个版本兼容，安装、启动、卸载、回滚证据分别记录。Registry CI 现在也覆盖运行代码与自动化脚本变更，并执行完整检查。DSH `0.1.5-alpha.1/.2` 的公开 Web、Slot、runtime 和安全事务契约已完成源码复核；公网发布和真实 Profile 验收仍是独立阶段。
