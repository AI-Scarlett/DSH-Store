...
`0.9.0` 复用官方 Connection 登录校验，增加持久化操作记录、真实回滚结果、生效状态、脱敏诊断、兼容筛选、本机收藏/备注及固定来源截图。外部目录仅提供待审候选，原有固定 Commit 审核及作者全局一次联系规则保持不变。DSH `0.1.5-rc.1`、`0.1.5-rc.2` 已通过隔离环境安装/启动/卸载验证；`0.1.6-alpha.1` 因官方依赖缺失保留 unknown。真实 Profile 升级必须先交接新版 Guardian，再启动需要认证探针的 Store；中断记录需根据备份制定恢复计划，系统不会重放操作。
