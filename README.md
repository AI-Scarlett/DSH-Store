...
`0.8.8` 修复远程 Catalog 增长到 2 MiB 以上后被旧商城拒绝、退回过期内置快照而导致插件列表为空或版本陈旧的问题。Catalog 现在采用轻量主索引 + 插件独立详情文件：`registry/catalog.json` 只承载插件编号、中英文名称、版本、推荐标记、顺序、GitHub 地址及有界的分页辅助字段；完整权限、兼容性、证据和安装信息位于 `registry/catalog/details/<插件编号>.json`，商城先取索引，再按当前页或详情操作懒加载。主索引响应上限为 2 MiB，详情文件单独限制 512 KiB；超过上限、网络失败或内容无效时仍失败关闭，不会伪装成在线目录。这个响应上限只针对 Catalog 传输，不放宽插件源码自动审查的 2 MiB 完整检查上限。
...
- v2 主索引响应保持 2 MiB、单个详情文件保持 512 KiB 的有界上限；旧 v1 Catalog 仍保留 4 MiB 兼容读取上限；超限、网络失败或内容无效时失败关闭并标记回退原因，不把过期快照伪装成在线目录；
...
`registry/catalog.json` 主索引与 `registry/catalog/details/<插件编号>.json` 详情文件共同构成权威目录；主索引负责定位和稳定分页，详情文件负责完整条目数据。
