...
- DeepSeek Harness `0.0.1-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7` 或 `0.1.0-rc.8`，并且官方 `dsh` CLI 可用；
...
`0.7.0` 将 DSH 内嵌商城和公共目录改为每页 24 条的有界加载，并将官方客户端契约兼容范围扩展到 rc.5–rc.8。
...
- 默认按照 approved、rc.8 兼容性、GitHub 固定来源更新时间、推荐和安装量依次排序；不支持 rc.8 或长期未更新的条目排在后面；
...
- DSH rc.5–rc.8 中使用同一套官方 `ModuleLoader`、`settings.plugins.tab`、Slots 和 Modal 契约；rc.5 的真实包版本是 `0.0.1-rc.5`；
...
- 对 rc.5、rc.6、rc.7、rc.8 分别展示兼容声明，以及安装、启动、卸载、回滚四项运行证据；缺少真实验收时显示未知；
...
- 分别在一次性 DSH `0.0.1-rc.5`、`0.1.0-rc.6`、`0.1.0-rc.7`、`0.1.0-rc.8` Profile 中使用官方 CLI 验证安装、配置合成和功能可见性；
