...
`0.8.1` 延续每页 24 条的有界加载、`0.1.1-rc.1` 兼容矩阵和来源排序，并新增所有标签页常驻的 Boot Guard：插件更新或 Guardian 重启后先校验新 Boot ID、Guardian `healthy` 状态和首页资源，连续稳定后再用带 Boot 参数的新 URL 恢复；`BroadcastChannel` 同步多标签页，轮询作为降级，同一 Boot 只导航一次。
