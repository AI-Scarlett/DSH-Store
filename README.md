...
- 已安装 `0.8.5`、`0.8.6` 或 `0.8.7` 的用户仍可读取只有数 KB 的 `registry/catalog.json`，并看到其中固定 Commit 的 `0.8.8` 商城更新；三个历史版本的原始 Catalog 验证器和更新卡片逻辑均有回归测试覆盖，不会再因 2 MiB 单体目录而回退到过期内置快照。
- 更新到 `0.8.8` 后，商城可读取过渡期单体目录，并能对紧随其后的 `0.8.9` 源版本生成固定 Commit 更新候选；当第二阶段 Catalog Pin PR 合并后，兼容桥会直接固定到 `0.8.9`。
