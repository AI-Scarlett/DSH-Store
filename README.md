...
`0.9.1` 复用官方 Connection 登录校验，增加持久化操作记录、真实回滚结果、生效状态、脱敏诊断、兼容筛选、本机收藏/备注及固定来源截图。外部目录仅提供待审候选，原有固定 Commit 审核及作者全局一次联系规则保持不变。DSH 官方最新三个受支持版本 `0.1.7-alpha.1`、`0.1.7-alpha.2`、`0.1.7-rc.1` 均已通过 GitHub 隔离 Profile 的安装、启动、卸载及精确恢复基线矩阵；Linux、macOS、Windows 与 pnpm 10/11/12 的当前版本组合通过。工作流会动态读取官方最新三个版本，不再固定在旧版 `0.1.5-rc.2`。本地 Profile 仍需单独升级，源码 PR 和 CI 通过不代表本机已安装或公开 Catalog 已完成发布。
