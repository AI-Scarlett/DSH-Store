...
`0.8.17` 继续同时读取官方 GitHub Releases 与 npm 已发布版本，并把 `dsh-v0.1.5-rc.1`（Commit `183f08e9c6dde7e36cd2318eaee70b0da08fb35e`）纳入官方版本权威。当前最新三版窗口为 `0.1.5-alpha.1`、`0.1.5-alpha.2`、`0.1.5-rc.1`；npm `latest` 与 `next` 均已指向 `0.1.5-rc.1`，因此商城会提供固定版本升级命令，但不会静默执行全局包管理器。任一权威来源失败时不推进自动兼容决策；精确兼容门仍要求窗口内至少一个版本兼容，安装、启动、卸载、回滚证据分别记录。Registry CI 现在也覆盖运行代码与自动化脚本变更，并执行完整检查。DSH `0.1.5-rc.1` 的公开 Web、Slot、runtime 和安全事务契约已完成源码复核；公网发布和真实 Profile 验收仍是独立阶段。
