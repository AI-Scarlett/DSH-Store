...
const translations = {
  zh: {
    ...
    'faq.q4': '插件作者只更新 GitHub，用户能收到提醒吗？', 'faq.a4': '可以。DSH 内的商城只在用户本机按需检查已安装插件的源仓库，先将默认分支解析为完整 Commit，再审核版本、Bundle、入口、生命周期与权限信号。低风险且契约未漂移时才能生成更新计划；高风险或不确定变化必须等待 Registry 复审。商城不会直接安装浮动 main，也不需要服务端巡检所有仓库。',
  ...
  en: {
    ...
    'faq.q4': 'Will users see an update when only the plugin repository changes?', 'faq.a4': 'Yes. The in-DSH market checks installed plugins on demand from the user device, resolves the default branch to a full commit, then checks version, bundle, entry IDs, lifecycle, and permission signals. Only low-risk candidates with an unchanged contract can produce an update plan; higher-risk or uncertain changes wait for Registry review. Floating main is never installed, and no server-wide repository crawler is required.',
