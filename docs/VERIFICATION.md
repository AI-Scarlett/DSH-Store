...
### `0.8.0` 到 `0.8.1` 在线目录恢复与 Settings Hub 推荐

- `0.8.0` 客户端要求每个显式 `dshReleases` 矩阵保留 rc.5、rc.6、rc.7、rc.8 和
  `0.1.1-rc.1` 键；Settings Hub 初次上架只声明 rc.1/rc.2，导致旧客户端拒绝整份在线
  Catalog 并回退到内置的 `0.7.0` 商城条目，因此同时出现“已安装 0.8.0”和目录版本
  `0.7.0`，也无法生成到 `0.8.1` 的更新计划；
- Settings Hub 对缺少运行证据的 rc.5–rc.8 保持 `unknown`，不把向后可解析误写成兼容；
  其 `featured` 调整为 `true`，推荐状态不会提高来源、安装、运行或安全审查等级；
- Catalog 契约测试现在要求所有显式兼容矩阵保留 `0.8.0` 客户端认识的基础键，防止新增
  DSH 版本或新条目再次让旧商城整体回退；真实 Profile 更新仍属于独立确认与 E4 验收。
