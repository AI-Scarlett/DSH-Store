...
export function createGuardianService(options = {}) {
  ...
  async function createInstallPlan(input = {}) {
    ...
    const plan = {
      schemaVersion: 1, planId, action: 'install-guardian', profile: input.profile ?? 'web',
      createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 300_000).toISOString(),
      confirmation: `INSTALL DSH GUARDIAN ${input.profile ?? 'web'}`,
      preconditions: { plistSha256: await fileDigest(plistPath), daemonSha256: await fileDigest(daemonSource) },
      impact: {
  ...
  async function executeInstall(input = {}) {
    ...
    return { schemaVersion: 1, status: 'installed', label, profile: plan.profile, plistPath }
  }
