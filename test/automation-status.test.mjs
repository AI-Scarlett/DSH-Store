...
test('missing workflow evidence remains explicitly unknown', () => {
  ...
  assert.equal(status.latestChanges.statisticsAvailable, false)
  assert.equal(status.latestChanges.added, null)
  assert.equal(status.latestChanges.sourceVersionChecks, null)
  assert.equal(status.recentAdditions.length, 0)
  assert.equal(status.recentUpdates.length, 0)
})

test('a failed latest scan exposes unavailable statistics while retaining completed historical changes', () => {
  const status = buildAutomationStatus({
    catalog, candidates: { entries: [] }, generatedAt: '2026-08-25T11:00:00Z', sourceCommit: 'a'.repeat(40),
    runs: {
      catalogAutomation: [
        { databaseId: 44, status: 'completed', conclusion: 'failure', createdAt: '2026-08-25T10:00:00Z', updatedAt: '2026-08-25T10:10:00Z', url: 'https://github.com/example/repo/actions/runs/44', headSha: 'd'.repeat(40) },
        { databaseId: 42, status: 'completed', conclusion: 'success', createdAt: '2026-08-25T02:00:00Z', updatedAt: '2026-08-25T02:10:00Z', url: 'https://github.com/example/repo/actions/runs/42', headSha: 'b'.repeat(40) },
      ],
      marketplaceWatchdog: [
        { databaseId: 45, status: 'completed', conclusion: 'failure', createdAt: '2026-08-25T10:55:00Z', updatedAt: '2026-08-25T10:56:00Z', url: 'https://github.com/example/repo/actions/runs/45', headSha: 'e'.repeat(40) },
      ],
    },
    reports: [
      { runId: 44, report: {
        status: 'failed', completed: false, statisticsAvailable: false,
        observedAt: '2026-08-25T10:01:00Z',
        sourceVersionChecks: { checkedEntries: 85, newerVersionCandidates: 12, catalogUpdates: 0 },
        addedEntries: [], updatedEntries: [],
        failure: { stage: 'apply-latest-dsh-compatibility-policy', message: 'compatibility alias conflict' },
      } },
      { runId: 42, report: {
        observedAt: '2026-08-25T02:01:00Z',
        addedEntries: [{ id: 'new-plugin' }], updatedEntries: [],
      } },
    ],
  })
  assert.equal(status.overall.status, 'failed')
  assert.equal(status.latestChanges.statisticsAvailable, false)
  assert.equal(status.latestChanges.added, null)
  assert.equal(status.latestChanges.updated, null)
  assert.equal(status.latestChanges.sourceVersionChecks, null)
