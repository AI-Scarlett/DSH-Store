...
test('static marketplace derives manager identity and catalog cards without mutating the authority file', async () => {
  ...
  try {
    ...
    assert.match(article, /hreflang="zh-CN"[^>]*https:\/\/dsh\.store\/about\/deepseek-harness-guide\//)
    assert.match(article, /hreflang="x-default"[^>]*https:\/\/dsh\.store\/about\/deepseek-harness-guide\//)
    assert.doesNotMatch(article, /dsh-store\.cn|site-switch-link/)
    assert.match(article, /作者：<a href="https:\/\/x\.com\/Russell3402"/)
    assert.match(about, /href="\.\/deepseek-harness-guide\/"/)
    assert.doesNotMatch(about, /DSH_INTL_ARTICLE_PROMO/)
...
test('static marketplace accepts a domestic origin and renders the ICP record', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(sitemap, /deepseek-harness-guide/)
    assert.doesNotMatch(sitemap, /https:\/\/dsh\.store/)
    assert.doesNotMatch(domesticAbout, /deepseek-harness-guide|Russell3402|DSH_INTL_ARTICLE_PROMO/)
    assert.equal(release.files['marketplace/about/deepseek-harness-guide/index.html'], undefined)
    await assert.rejects(
      readFile(join(output, 'marketplace/about/deepseek-harness-guide/index.html'), 'utf8'),
